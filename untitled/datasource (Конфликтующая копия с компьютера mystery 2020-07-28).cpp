...
void DataSource::init()
{
    ...
    {
            // делаешь что нужно
        fin.open("datasource/uaa.txt",std::ios::in);
        fin >> X1_1 >> ua;

        fin5.open("datasource/ubb.txt",std::ios::in);
        fin5 >> X5_1 >> ub;

        fin2.open("datasource/iaa.txt",std::ios::in);
        fin2 >> X2_1 >> ia ;

        fin3.open("datasource/ibb.txt",std::ios::in);
        fin3 >> X3_1 >> r ;
        //fin2.close();

        fin4.open("datasource/ww.txt",std::ios::in);
        fin4 >> X4_1 >> w;
        //X4_1=0;
        //w=0;
