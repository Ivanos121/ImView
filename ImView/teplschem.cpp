...
void teplschem::on_checkBox_2_toggled(bool checked)
{
    if (checked)
    {
       // wf->ui->widget_5->ui->widget_3->ui->webEngineView ->page()->runJavaScript(QString("$('#text255').text('%1 °C');").arg(trend.y_8, 0, 'f', 1));
        ui->webEngineView ->page()->runJavaScript(QString("$('#text255').text('%1 °C');").arg(trend.y_8, 0, 'f', 1));
    }
    else
    {
